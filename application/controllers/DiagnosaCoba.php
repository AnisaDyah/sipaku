...
class DiagnosaCoba extends CI_Controller {
	...
	public function index()
	{
        ...
        if($this->input->cookie('cookie_id_gejala[0]',true) != null){
            foreach ($this->input->cookie('cookie_id_gejala',true) as $key => $value) {
                ...
                delete_cookie("cookie_id_gejala_tidak[".$key."]");
            }
        }
        ...
        foreach ($basis_kasus as $key => $value) {
            ...
            if($search !== null){
                ...
                if(count($gejala_kasus_updated) !== 0){
                    $cookie_id_gejala= array(
                        'name'   => 'cookie_id_gejala['.($array_id+1).']',
    ...
    public function cek_gejala_tidak($id_gejala)
	{
        ...
        //set cookie untuk id gejala tidak terpilih 
        $id_gejala_toPush = $this->GejalaModel->get_id_gejala($id_gejala)->result_array()[0]['id_gejala'];
            $cookie_id_gejala_tidak= array(
                'name'   => 'cookie_id_gejala_tidak['.$array_id.']',
                'value'  => $id_gejala_toPush,                            
                'expire' => '3600',                                                                                   
                'secure' => true
            );
            $this->input->set_cookie($cookie_id_gejala_tidak);

        $search=null;
        //cek sudah ada basis kasus yang pernah diolah atau belum
        if($this->input->cookie('cookie_basis_kasus',true) == null){
            $basis_kasus =$this->BasisKasusModel->get_basis_kasus();
            ...
            // cek dulu apakah masih ada gejala lain?
            $total_gejala_asli =$this->BasisKasusModel->get_basis_kasus_byid(json_decode($this->input->cookie('cookie_basis_kasus',true))[0]->id_penyakit);
            //echo $total_gejala_asli;
            $total_gejala = round(count($total_gejala_asli) / 2);
            if(count(json_decode($this->input->cookie('cookie_basis_kasus',true))[0]->gejala[0]) > $total_gejala){
                $basis_kasus=[];
                $data= json_decode($this->input->cookie('cookie_basis_kasus',true));
                foreach ($data as $key => $value) {
                $objectToArray1 = (array)$value;
    ...
    public function hasil_diagnosa()
	{
        ...
        //array id gejala iya
        $array_id_gejala_cookie=[];
        $array_id_gejala_cookie_tidak=[];
        if($this->input->cookie('cookie_id_gejala[0]',true) != null){
            foreach ($this->input->cookie('cookie_id_gejala',true) as $key => $value) {
                array_push($array_id_gejala_cookie,$value);
            }
            foreach ($this->input->cookie('cookie_id_gejala_tidak',true) as $key => $value) {
                array_push($array_id_gejala_cookie_tidak,$value);
            }
        }
        $array_id_gejala = array_unique($array_id_gejala_cookie);
        var_dump($array_id_gejala);

        //array id gejala tidak
        $array_id_gejala_tidak = array_unique($array_id_gejala_cookie_tidak);
        //var_dump($array_id_gejala_tidak);

        $fix_id_gejala = array_diff($array_id_gejala,$array_id_gejala_tidak);
        var_dump($fix_id_gejala);
        if(count($fix_id_gejala) == 0){
            $fix_id = $array_id_gejala;
        }else{
            $fix_id = $fix_id_gejala;
        }
        ...
        foreach ($basis_kasus_byid as $key => $value) {
            foreach ($fix_id as $va) {
                if($value['id_gejala'] === $va){
                    array_push($gejala_selected,$value);
                }
            }
        }
        $perhitungan_fc = (count($gejala_selected)/count($basis_kasus_byid)) * 100;
